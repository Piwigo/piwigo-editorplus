...
$(document).ready(function () {
        ...
        $.ajax({
            ...
            success: function(res) {
                ...
                if (result.status == 'success') {
                    EP_CONFIG_EDITOR.config_quill = result.configuration.config_quill;
                }
    ...
    $('.load-quill').hide();
    if ('quill' === EP_CONFIG_EDITOR.editor) {
        $('.config-quill-container').fadeIn();
    }

    $('.ep-editor p').on('click', function () {
        const quillOption = $('.config-quill-container, #ep_config_option');
        const ckeditorOption = $('.config-ckeditor-container');

        $('.ep-editor p').removeClass('choosen');
        $(this).addClass('choosen');

        if ('quill' === $(this).data('value')) {
            ckeditorOption.hide();
            quillOption.fadeIn();
        } else {
            quillOption.hide();
            ckeditorOption.fadeIn();
        }
        
        newEditor = $(this).data('value');
    });
});